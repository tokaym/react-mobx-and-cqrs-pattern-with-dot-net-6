...
public class GetOpenAmountbyCompanyQuery : IRequest<List<BarChartDto>>
{
    public PageRequest? PageRequest { get; set; }
    public OrderByRequest? OrderByRequest { get; set; }
    public string PlantCode { get; set; }
    public class GetOpenAmountbyCompanyQueryHandler : IRequestHandler<GetOpenAmountbyCompanyQuery, List<BarChartDto>>
    {
        private readonly IMainReportRepository _mainReportRepository;
        private readonly IMapper _mapper;
        public GetOpenAmountbyCompanyQueryHandler(IMainReportRepository mainReportRepository, IMapper mapper)
        {
            _mainReportRepository = mainReportRepository;
            _mapper = mapper;
        }

        public async Task<List<BarChartDto>> Handle(GetOpenAmountbyCompanyQuery request, CancellationToken cancellationToken)
        {
            ...
            List<BarChartDto> charts = report
                        .GroupBy(a => new { a.Company })
                        .Select(r => new BarChartDto
                        {
                            Name = r.First().Company,
